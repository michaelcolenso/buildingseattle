...
async function renderPermitDetail(permitNumber, env, request) {
  ...
  const statusLabel = cleanFeedText(permit.status) || "under review";
  const permitTitle = `${permitAddressLabel} — ${permitDescriptor || permitType} | Permit ${cleanFeedText(permit.permit_number) || "record"} | Building Seattle`;
  const metaDesc = truncateMetaDescription(
    `${permitDescriptor ? `${permitDescriptor} at ${addressLabel}. ` : `See ${addressLabel}: `}${permitType} permit ${cleanFeedText(permit.permit_number) || "record"} in ${neighborhood}. Status: ${statusLabel}. Value: ${valueFormatted}.${permit.contractor_name ? ` Contractor: ${permit.contractor_name}.` : ""}`,
  );
  ...
  const safeAddress = escapeHtml(permitAddressLabel || "Unknown Address");
  ...
  const firstScreenSummary = permitDescriptor || `${permitType} permit in ${neighborhood}`;
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>${escapeHtml(permitTitle)}</title>
	    <meta name="description" content="${safeMetaDesc}">
    <meta name="robots" content="index,follow,max-image-preview:large">
    <link rel="canonical" href="${canonical}">
    <meta property="og:title" content="${escapeHtml(permitTitle)}">
    ...
    <main>
        <div class="container">
            <div class="permit-header">
	                ...
	                <p style="margin:0 0 0.85rem;max-width:760px;font-size:1.05rem;line-height:1.55;color:var(--text);"><strong>${escapeHtml(firstScreenSummary)}</strong> <span style="color:var(--text-muted);">· ${safePermitType} · Permit ${safePermitNumber}</span></p>
	                ${firstScreenLinks ? `<nav aria-label="Related permit records" style="display:flex;flex-wrap:wrap;gap:0.5rem 0.9rem;margin:0 0 1rem;font-size:0.84rem;">${firstScreenLinks}</nav>` : ""}
